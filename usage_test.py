...
if __name__ == "__main__":
    ...
    # Create the dataset
    dataset = Dataset("dataset", "no_obj", vae_model, seq_len=300)
    ...
    lstm_model = MuLogvarLSTM(embedding_dim=latent_dim, hidden_dim=256, num_layers=2, dropout=0.1).to(device)
    ...
    # take a sample from the dataset
    sample_mu = sample_mu[:, :10, :].float().to(device)
    sample_logvar = sample_logvar[:, :10, :].float().to(device)
    sample_act = sample_act.float().to(device)
    mu, logvar = lstm_model.predict(sample_mu, sample_logvar, sample_act)
